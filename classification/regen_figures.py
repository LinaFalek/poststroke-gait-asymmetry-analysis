...
df_lr    = pd.read_csv(DATA_DIR / 'features_ailr.csv')
df_stats = pd.read_csv(DATA_DIR / 'feature_statistics.csv')
df_preds = pd.read_csv(RESULTS_DIR / 'LOSO_predictions.csv')
df_comp  = pd.read_csv(RESULTS_DIR / 'model_comparison.csv')
df_sweep = pd.read_csv(RESULTS_DIR / 'ksweep_results.csv')
df_perm  = pd.read_csv(RESULTS_DIR / 'permutation_null.csv')

MODEL_NAMES = ['Logistic Reg', 'SVM Linear', 'Random Forest', 'Naive Bayes']
le          = LabelEncoder()
le.fit(df_preds['true_label'])
y_true      = le.transform(df_preds['true_label'])

FEAT_COLS = ['walkingSpeed'] + [c for c in df_lr.columns if c.startswith('AILR_')]
n_feats   = len(FEAT_COLS)

obs_k18  = df_comp[df_comp['model'] == f'SVM Linear (k={n_feats})']['bal_acc'].values[0]
obs_k2   = df_comp[df_comp['model'] == 'SVM Linear']['bal_acc'].values[0]
null_k18 = df_perm['null_k18'].values
null_k2  = df_perm['null_k2'].values
p_k18    = (null_k18 >= obs_k18).mean()
p_k2     = (null_k2  >= obs_k2).mean()

k_values      = df_sweep['k'].tolist()
sweep_results = {
    'SVM Linear':   df_sweep['SVM Linear'].tolist(),
    'Logistic Reg': df_sweep['Logistic Reg'].tolist(),
}

results_k2 = {}
for name in MODEL_NAMES:
    preds   = le.transform(df_preds[name])
...
print('[1/9] dataset_overview.png ...')
...
print('[2/9] feature_statistics.png ...')
top_n = 20
...
print('[3/9] top_feature_distributions.png ...')
...
print('[4/9] confusion_matrices.png ...')
...
print('[5/9] model_comparison.png ...')
...
print('[6/9] feature_selection_frequency.png (k-sweep) ...')
...
print('[7/9] permutation_test.png ...')
...
print('[8/9] psd_filter_justification.png ...')
FS = 100; FC = 10
...
print('[9/9] swing_detection_thresholds.png ...')
np.random.seed(42)
